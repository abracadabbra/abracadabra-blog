...
description: '让数据说话，让 AI 辩论，让决策有据可依。一个融合 200+ 技术指标、Prophet 时序预测与 4 Agent 多角色辩论的黄金分析实战项目。'
pubDate: '2026-05-18'
tags: ['AI', 'Agent', '量化交易', 'LLM', 'Python', 'FastAPI', '黄金', '多Agent辩论', 'Prophet', 'backtrader']
...
![首页](https://raw.githubusercontent.com/abracadabbra/gold-agent/master/frontend/public/index.png)

**数据面板 — 实时行情与技术指标**

![数据面板](https://raw.githubusercontent.com/abracadabbra/gold-agent/master/frontend/public/dashboard.png)
...
│  │  数据采集    │ │  量化分析    │ │    LLM 辩论引擎          │ │
│  │             │ │             │ │                          │ │
│  │ · akshare   │ │ · pandas-ta │ │ · 🟢 看多方 (GPT-5.5)      │ │
│  │ · yfinance  │ │ · Prophet   │ │ · 🔴 看空方 (Claude 4.7)   │ │
│  │ · FRED API  │ │ · backtrader│ │ · 🔍 审计员 (GPT-5.5-mini) │ │
│  │ · RSS 新闻  │ │ · 信号生成  │ │ · ⚖️ 仲裁官 (GPT-5.5)     │ │
│  └──────┬──────┘ └──────┬──────┘ └────────────┬────────────┘ │
│         │               │                     │              │
│  ┌──────▼───────────────▼─────────────────────▼────────────┐ │
│  │                    共享数据层                              │ │
...
└──────────────┬─────────────────────────┘
               ...
               ▼
┌─ Step 3: 辩论 Round 1 ─────────────────┐
│  🟢 看多方: 基于数据构建看多论点 (GPT-5.5)       │
│  🔴 看空方: 基于数据构建看空论点 (Claude 4.7 Opus) │
└──────────────┬─────────────────────────┘
               │
               ▼
┌─ Step 4: 数据审计 ─────────────────────┐
│  🔍 审计员: 验证双方数据准确性 (GPT-5.5-mini)    │
└──────────────┬─────────────────────────┘
               │
               ▼
┌─ Step 5: 仲裁裁决 ─────────────────────┐
│  ⚖️ 仲裁官: 综合所有输入输出最终判断 (GPT-5.5)    │
└──────────────┬─────────────────────────┘
               │
               ▼
         结构化 JSON 输出
...
| Agent | 角色 | 模型 | 温度 | 职责 |
|-------|------|------|------|------|
| 🟢 看多方 | Bull Advocate | GPT-5.5 | 0.7 | 构建看多论点，引用数据支撑 |
| 🔴 看空方 | Bear Challenger | Claude 4.7 Opus | 0.7 | 质疑反驳，寻找反面证据 |
| 🔍 审计员 | Data Auditor | GPT-5.5-mini | 0.2 | 验证数据准确性，标记幻觉 |
| ⚖️ 仲裁官 | Chief Arbitrator | GPT-5.5 | 0.4 | 综合裁决，输出趋势判断 |

**为什么用不同模型？**
- **看多方用 GPT-5.5**：擅长数据引用和逻辑构建
- **看空方用 Claude 4.7 Opus**：Claude 更擅长批判性思维和质疑
- **审计员用 GPT-5.5-mini**：成本低，专注于事实核查
- **仲裁官用 GPT-5.5**：综合能力强，适合最终裁决
...
首页展示 **7 大数据维度**：行情数据、宏观指标、国内外新闻、技术分析、AI 预测概率、情景推演、风险提示。
...
项目使用 **Trellis** 进行任务管理，具备完整的工程化配置：

- **CI/CD**: GitHub Actions 自动化流水线
- **数据库迁移**: Alembic 版本化迁移
- **单元测试**: 16 个测试文件，覆盖数据层、量化层、辩论层、API 层
- **集成测试**: FastAPI TestClient 端到端验证
- **代码规范**: ruff lint + mypy 类型检查
...
### 4. pandas-ta 依赖问题
pandas-ta 在某些环境下编译失败。解决方案：
- 代码中内置纯 pandas 回退实现
- 优先使用 pandas-ta，不可用时自动降级
...
│
├── tests/                     # pytest 单元测试 (16 个测试文件)
├── frontend/                  # Next.js 前端
├── alembic/                   # 数据库迁移
├── .trellis/                  # Trellis 工作流管理
